...
static inline uint16_t get_pixel_from_planar_with_padding(
	int y, int x,
	int src_full_h, int src_full_w,
	const uint8_t *src_msb_plane,
	const uint8_t *src_lsb_plane,
	int blc)
{
	if (y < 0) {
		y = -y - 1;
...
void ainr_dump_time_stats(ainr_ctx_t *ctx)
{
	...
	printf("  Patch Processing: (min: %lu ms, max: %lu ms, avg: %lu ms, count: %lu, total: %lu ms)\n",
		   stats->patch.min_time,
		   stats->patch.max_time,
		   stats->patch.count ? stats->patch.total_time / stats->patch.count : 0,
		   stats->patch.count,
		   stats->patch.total_time);
	printf("  uLaw Encode: (min: %lu ms, max: %lu ms, avg: %lu ms, count: %lu, total: %lu ms)\n",
		   stats->ulaw_encode.min_time,
		   stats->ulaw_encode.max_time,
		   stats->ulaw_encode.count ? stats->ulaw_encode.total_time / stats->ulaw_encode.count : 0,
		   stats->ulaw_encode.count,
		   stats->ulaw_encode.total_time);
	...
	printf("  Mixup: (min: %lu ms, max: %lu ms, avg: %lu ms, count: %lu, total: %lu ms)\n",
		   stats->mixup.min_time,
		   stats->mixup.max_time,
		   stats->mixup.count ? stats->mixup.total_time / stats->mixup.count : 0,
		   stats->mixup.count,
		   stats->mixup.total_time);
	printf("  uLaw Decode: (min: %lu ms, max: %lu ms, avg: %lu ms, count: %lu, total: %lu ms)\n",
		   stats->ulaw_decode.min_time,
		   stats->ulaw_decode.max_time,
		   stats->ulaw_decode.count ? stats->ulaw_decode.total_time / stats->ulaw_decode.count : 0,
		   stats->ulaw_decode.count,
		   stats->ulaw_decode.total_time);
	...
	unsigned long total_time = stats->patch.total_time +
							   stats->ulaw_encode.total_time +
							   stats->nn_inference.total_time +
							   stats->mixup.total_time +
							   stats->ulaw_decode.total_time +
...
static void extract_and_convert_patch_from_planar(
	uint16_t *dst_nchw_patch,
	int patch_h, int patch_w,
	const uint8_t *src_msb_plane,
	const uint8_t *src_lsb_plane,
	int src_full_h, int src_full_w,
	int y_nchw, int x_nchw,
	int blc)
{
	...
	for (int y_p = 0; y_p < patch_h; ++y_p) {
		for (int x_p = 0; x_p < patch_w; ++x_p) {
			int src_y_base = (y_nchw + y_p - OVERLAP_H) * 2;
			int src_x_base = (x_nchw + x_p - OVERLAP_W) * 2;

			uint16_t gr_val = get_pixel_from_planar_with_padding(src_y_base,     src_x_base,     src_full_h, src_full_w, src_msb_plane, src_lsb_plane, blc);
			uint16_t r_val  = get_pixel_from_planar_with_padding(src_y_base,     src_x_base + 1, src_full_h, src_full_w, src_msb_plane, src_lsb_plane, blc);
			uint16_t b_val  = get_pixel_from_planar_with_padding(src_y_base + 1, src_x_base,     src_full_h, src_full_w, src_msb_plane, src_lsb_plane, blc);
			uint16_t gb_val = get_pixel_from_planar_with_padding(src_y_base + 1, src_x_base + 1, src_full_h, src_full_w, src_msb_plane, src_lsb_plane, blc);
...
static void stitch_nchw_patch_to_packed_bayer(
	uint16_t *dst_packed_bayer,
	int dst_h, int dst_w,
	const uint16_t *src_nchw_patch,
	int patch_h, int patch_w,
	int y_nchw, int x_nchw,
	int blc)
{
...
static int denoise_tiled(ainr_ctx_t *ctx, uint16_t *denoised_packed_bayer_output,
						 const uint8_t *src_msb_plane, const uint8_t *src_lsb_plane, int blc)
{
	...
	for (int y_orig = 0; y_orig < h_packed; y_orig += stride_h) {
		for (int x_orig = 0; x_orig < w_packed; x_orig += stride_w) {
			...
			ctx->time_stats.patch.start_time = xTaskGetTickCount();
			extract_and_convert_patch_from_planar(
				patch_input_u16,
				MODEL_INPUT_SIZE_H, MODEL_INPUT_SIZE_W,
				src_msb_plane, src_lsb_plane,
				full_h, full_w,
				y_orig, x_orig,
				blc);
			update_time_stats(&(ctx->time_stats.patch), xTaskGetTickCount());

			ctx->time_stats.ulaw_encode.start_time = xTaskGetTickCount();
			ainr_ulaw_encode_fast(patch_input_u16, patch_input_encoded_u8, patch_elements);
			update_time_stats(&(ctx->time_stats.ulaw_encode), xTaskGetTickCount());
			...
			ctx->time_stats.mixup.start_time = xTaskGetTickCount();
#if MIXUP_FACTOR == MIXUP_FACTOR_025
			for (size_t i = 0; i < patch_elements; ++i) {
				patch_denoised_u8[i] = ((uint16_t)(patch_denoised_u8[i]) * 3 + patch_input_encoded_u8[i]) >> 2;
			}
#else
#error "[ainr] invalid mix factor"
#endif
			update_time_stats(&(ctx->time_stats.mixup), xTaskGetTickCount());

			ctx->time_stats.ulaw_decode.start_time = xTaskGetTickCount();
			ainr_ulaw_decode_fast(patch_denoised_u8, patch_denoised_decoded, patch_elements);
			update_time_stats(&(ctx->time_stats.ulaw_decode), xTaskGetTickCount());

			ctx->time_stats.tile.start_time = xTaskGetTickCount();
			stitch_nchw_patch_to_packed_bayer(
				denoised_packed_bayer_output,
				full_h, full_w,
				patch_denoised_decoded,
				MODEL_INPUT_SIZE_H, MODEL_INPUT_SIZE_W,
				y_orig, x_orig,
				blc);
